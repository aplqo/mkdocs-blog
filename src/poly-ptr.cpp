...
constexpr unsigned long long mod = 998244353, primitiveRoot = 3, inv2 = 499122177;
...
#undef decl
    ...
    inline bool operator==(const unsigned long long v) const
    {
        return val == v;
    }
...
void convolution(const ConstArrayPtr &a, const ConstArrayPtr &b, const ArrayPtr dest, const Func pred)
{
...
void sqrt(const ConstArrayPtr a, const ArrayPtr dest)
{
    ...
    static const auto updateSqrt = [](const ConstArrayPtr a, Number dest[], const unsigned int n) {
...
namespace Product
{
    ...
    inline void multiply(ConstArrayPtr &now, const ConstArrayPtr *left, Number *&cur)
    {
        ArrayPtr tp;
        if (inPlace)
    ...
    void product(const ConstArrayPtr a, const unsigned int lgn, ConstArrayPtr *const prod, Number *&cur, Func ini)
    {
...
namespace Evaluation
{
    ...
    ConstArrayPtr transMultiply(const Number vx[], const ConstArrayPtr &b, const unsigned int lgn)
    {
    ...
    void conquer(Number *const dest, const unsigned int lgn)
    {
        ...
        for (unsigned int cid = 1; cid < n; ++cid)
        {
            ...
            const unsigned int childSize = t[cid].size + std::max(prod[left].size, prod[right].size) - 1, lgn = ceilLg[childSize];
    ...
    void evaluation(const ConstArrayPtr &a, const ConstArrayPtr &pos, Number dest[])
    {
        ...
        cur = mem;
        const unsigned int len = std::max(a.size, pos.size), lgn = ceilLg[len], n = 1u << lgn;
        Product::product<false>(pos, lgn, prod, cur, [](Number *cur, Number v) { cur[0] = 1; cur[1] = -v; });
        {
